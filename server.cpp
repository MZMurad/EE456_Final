...
#define PORT 8080 // Define the port number to be used for the server
int main() {
	...
	// The second argument specifies the maximum length of the queue for pending connections (3 in this case)
	if (listen(server_fd, 3) < 0) {
		perror("listen"); // Print an error message if the listen call fails
		exit(EXIT_FAILURE); // Exit the program with a failure status
	}
	// If the server successfully starts listening, print a message indicating it is ready
	cout << "Listening for connections..." << endl;




	int new_socket; // Variable to hold the file descriptor for the new connection
	...
	cout << "Message from client: " << buffer << endl; // Print the received message
